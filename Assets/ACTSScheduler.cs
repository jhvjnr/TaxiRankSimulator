...
public class ACTSScheduler : MonoBehaviour {

   

    public GameObject exampleTaxi;
  ...
  //  public static LinkedList<GameObject> aiChars = new LinkedList<GameObject>();
    ...
    void Start ()
    {
        ...
        WheelFrictionCurve tempForwardFrictionCurve = exampleTaxi.GetComponentInChildren<WheelCollider>().forwardFriction;
        WheelFrictionCurve tempSidewaysFrictionCurve = exampleTaxi.GetComponentInChildren<WheelCollider>().sidewaysFriction;
        tempForwardFrictionCurve.stiffness = frictionModification;
        tempSidewaysFrictionCurve.stiffness = frictionModification;
        var wheelColliders = exampleTaxi.GetComponentsInChildren<WheelCollider>();
        ...
        while (!taxiReader.EndOfStream)
        {
            ...
            GameObject newTaxi = Instantiate(exampleTaxi);
           ...
           // print(taxiToAddScript.ArrivalTime);
            ...
            newTaxi.transform.SetPositionAndRotation(new Vector3(50, 0, 160), exampleTaxi.transform.rotation);
...
/*
    ...
    private IEnumerator TaxiSpawn(GameObject taxi, float time)
    {
    ...
    private void SpawnCommutersForTaxi(int number, Destination destination, float time)
    {
    ...
    private IEnumerator ResetSimulation(float time)
    {
        ...
        private IEnumerator SpawnCommuter(Destination destination, float time)
    {
